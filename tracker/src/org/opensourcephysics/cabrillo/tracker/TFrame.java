...
//		
	...
	/**
	 * True to open the right-hand view pane (plot and table) at startup so that a
	 * newly tracked mass immediately shows its plots. Set false for the legacy
	 * behavior in which only the video view is shown until a view is chosen.
	 */
	public static boolean showRightViewsOnStartup = true;
...
//		isLayoutAdaptive = (options.get("-adaptive") != null);
		...
		if (bounds == null) {
			if (dim == null) {
				...
				// Size the initial window to the available screen area. The old
				// formula (min(0.9*w, 1024+extra*800)) collapsed to a cramped
				// 1024x768 on any modern display, which forced every panel--video,
				// plot and the frame slider--into a tiny area.
				int w = (int) (screenRect.width * 0.9);
				int h = (int) (screenRect.height * 0.9);
				// prefer a 4:3 window, but never exceed the screen
				if (3 * w > 4 * h) {
					w = 4 * h / 3;
				} else {
					h = 3 * w / 4;
				}
				// grow a little when the user has chosen larger fonts
				w = Math.min(screenRect.width, (int) (w * (1 + 0.1 * extra)));
				h = Math.min(screenRect.height, (int) (h * (1 + 0.1 * extra)));
				// keep a usable minimum for small screens
				w = Math.max(w, Math.min(1024, screenRect.width));
				h = Math.max(h, Math.min(700, screenRect.height));
		...
		pack();
		// pack() snaps the window to its (small) preferred content size, which
		// ignores the computed dim. Size the frame explicitly so the initial
		// window actually uses the available screen area.
		setSize(dim);
...
//		}
	...
	private void initialize(TrackerPanel trackerPanel) {
		...
		if (trackerPanel.dividerLocs == null) {
			// Show the right-hand views (plot and table) by default. The old code
			// set SPLIT_MAIN_RIGHT to 1.0 here, which collapsed the plot pane on
			// startup so a newly tracked mass appeared to have no plots at all.
			setDividerLocation(trackerPanel, SPLIT_MAIN_RIGHT,
					(portrait || !showRightViewsOnStartup) ? 1.0 : DEFAULT_MAIN_DIVIDER);
			setDividerLocation(trackerPanel, SPLIT_PLOT_TABLE, DEFAULT_RIGHT_DIVIDER);
