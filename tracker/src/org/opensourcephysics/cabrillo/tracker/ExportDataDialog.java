...
public class ExportDataDialog extends JDialog {
	...
	private boolean asPython = false;
	private static final String PYTHON_FORMAT_LABEL = "Python (.py)";
...
//		allColumnsMap.clear();
	...
	private void setFormatAction(String format) {
		if (PYTHON_FORMAT_LABEL.equals(format)) {
			asPython = true;
		} else {
			asPython = false;
	...
	private void saveAsAction() {
		...
		if (XML.getExtension(file.getName()) == null) {
			file = new File(file.getAbsolutePath() + (asPython? ".py": ".txt")); //$NON-NLS-1$
		}
		// For the Python export we write a plain text file, so skip the
		// video-writability check (which only applies to video containers).
		if (!asPython && !VideoIO.canWrite(file)) {
			JOptionPane.showMessageDialog(frame,
					"The selected file format cannot be written as a video:\n" + file.getAbsolutePath()
							+ "\n\nChoose a supported video container (e.g. .mp4, .avi, .mov) or export as text/Python.", //$NON-NLS-1$
					"Unsupported Format", //$NON-NLS-1$
...
//				buf.append(TeXParser.removeSubscripting(selectedColumnNames[j]) + units);
	...
	/**
	 * Builds a Python (.py) source string that exports the selected track data
	 * as NumPy-ready arrays. This is a separate, additive export path: it does
	 * not alter the existing text/clipboard export.
	 *
	 * @return the Python source, or null if there is nothing to export
	 */
	@SuppressWarnings("unchecked")
	private String getPythonString() {
		TrackerPanel panel = frame.getTrackerPanelForID(panelID);
		BitSet selectedTracksBS = getSelectedTracksBitSet(panelID);
		if (selectedTracksBS.isEmpty())
			return null;
		...
		double[][] frameNumbers = new double[selectedTrackCount][];
		for (int i = 0; i < selectedTrackCount; i++)
			frameNumbers[i] = getPoints(selectedTrackData[i].get(frameIndex), isFrames);
		...
		if (hasData) {
			for (int i = 0; i < selectedTrackCount; i++) {
				...
				int nn = 0;
				for (int j = min; j <= max; j++) {
					if (frameNumbers[i].length > 0 && frameNumbers[i][nn] == j) {
						indices[i][j - min] = nn;
						nn++;
						if (nn >= frameNumbers[i].length)
							break;
					}
				}
			}
		...
		sb.append("\"\"\"\n");
		sb.append("Tracker data export\n");
		sb.append("===================\n");
		sb.append("Generated by Tracker (Open Source Physics).\n");
		sb.append("Import this file, then use `metadata`, the shared independent\n");
		sb.append("variable, the per-track dicts, and the `tracks` mapping.\n");
		sb.append("Each per-track value is a float array aligned to the shared\n");
		sb.append("independent variable (np.nan where the track has no data).\n");
		sb.append("\"\"\"\n");
		sb.append("import numpy as np\n\n");

		sb.append("metadata = {\n");
		sb.append("    \"source\": \"Tracker (Open Source Physics)\",\n");
		sb.append("    \"n_tracks\": ").append(selectedTrackCount).append(",\n");
		sb.append("    \"time_variable\": ").append(pyQuote(isFrames? "t": "n")).append(",\n");
		sb.append("    \"time_unit\": ").append(pyQuote(isFrames? "s": "frame index")).append(",\n");
		sb.append("    \"frame_min\": ").append(hasData? min: 0).append(",\n");
		sb.append("    \"frame_max\": ").append(hasData? max: 0).append(",\n");
		sb.append("    \"tracks\": [\n");
		for (int i = 0; i < selectedTrackCount; i++) {
			sb.append("        {\"name\": ").append(pyQuote(selectedTracks[i].getName())).append(", \"columns\": [");
			for (int k = 0; k < colsPerTrack; k++) {
				if (k > 0)
					sb.append(", ");
				sb.append(pyQuote(selectedColumnNames[k]));
			}
			sb.append("]},\n");
		}
		sb.append("    ],\n");
		...
		String xIdent = pyIdent(xVar);
		sb.append("# Shared independent variable (").append(isFrames? "seconds": "frame index").append("), one entry per frame in [").append(hasData? min: 0).append("..").append(hasData? max: 0).append("]\n");
		sb.append(xIdent).append(" = np.array([");
		if (hasData) {
			for (int f = min; f <= max; f++) {
				if (f > min)
					sb.append(", ");
				sb.append(pyNum(independentValue(isFrames, player, f)));
			}
		}
		sb.append("], dtype=float)\n\n");

		for (int i = 0; i < selectedTrackCount; i++) {
			String tIdent = "track_" + (i + 1);
			sb.append(tIdent).append(" = {\n");
			for (int k = 0; k < colsPerTrack; k++) {
				String colName = selectedColumnNames[k];
				if (colName == null)
					continue;
				String cIdent = pyIdent(colName);
				sb.append("    ").append(pyQuote(cIdent)).append(": np.array([");
				if (hasData) {
					boolean isXCol = colName.equals(xVar);
					for (int f = min; f <= max; f++) {
						if (f > min)
							sb.append(", ");
						double value;
						if (isXCol) {
							value = independentValue(isFrames, player, f);
						} else {
							int idx = indices[i][f - min];
							if (idx > -1 && dataValues[i][k] != null && idx < dataValues[i][k].length)
								value = dataValues[i][k][idx];
							else
								value = Double.NaN;
						}
						sb.append(pyNum(value));
					}
				}
				sb.append("], dtype=float),\n");
			}
		...
		sb.append("tracks = {\n");
		for (int i = 0; i < selectedTrackCount; i++) {
			sb.append("    ").append(pyQuote(selectedTracks[i].getName())).append(": track_").append(i + 1).append(",\n");
		}
		sb.append("}\n");
	...
	/** Independent-variable value for a global frame (seconds for video, index otherwise). */
	private static double independentValue(boolean isFrames, VideoPlayer player, int f) {
		if (isFrames && player != null)
			return player.getFrameTime(f) / 1000.0;
		return (double)f;
	}

	/** Quote a string as a Python string literal (escape-safe). */
...
/** Format a double as a valid Python float literal. */
	private static String pyNum(double v) {
		if (Double.isNaN(v))
			return "np.nan";
	...
	/** Convert an arbitrary column name into a safe Python identifier. */
	...
	private JPopupMenu getFormatMenu() {
		...
		popup.add(item);
		// direct Python export (NumPy-ready .py)
		item = new JMenuItem(PYTHON_FORMAT_LABEL);
		item.addActionListener((e) -> {
			setFormatAction(PYTHON_FORMAT_LABEL);
		});
		popup.add(item);
	...
	private void refreshGUI() {
		...
		String form = TrackerRes.getString("TableTrackView.MenuItem.Formatted");
		String unform = TrackerRes.getString("TableTrackView.MenuItem.Unformatted");
		formatButton.setText(asPython? PYTHON_FORMAT_LABEL: (asFormatted? form: unform));	
	...
	private String write(File file, String content) {
		...
		// try-with-resources guarantees the stream is closed even if the write
		// fails part-way (previously a mid-write exception could leak the stream).
		try (FileOutputStream stream = new FileOutputStream(file);
				Writer out = new BufferedWriter(new OutputStreamWriter(stream, java.nio.charset.StandardCharsets.UTF_8))) {
			out.write(content);
			out.flush();
		} catch (Exception ex) {
			// Surface the failure to the user instead of only printing to the
			// console (a silent export failure is the main stability complaint).
			ex.printStackTrace();
			JOptionPane.showMessageDialog(frame,
					"Could not write export file:\n" + file.getAbsolutePath() + "\n\n" + ex.getMessage(), //$NON-NLS-1$
					"Export Failed", //$NON-NLS-1$
					JOptionPane.ERROR_MESSAGE);
			return null;
		}
		// Verify the file was actually created with content, not just that it
		// exists (guards against a zero-byte / truncated write).
