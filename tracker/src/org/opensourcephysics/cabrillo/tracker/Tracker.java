...
//		splash.setLocation(x - size.width / 2, y - size.height / 2);
	...
	/**
	 * Applies a modern Look &amp; Feel.
	 * <p>
	 * If the user has explicitly chosen a look &amp; feel (stored in
	 * {@link #lookAndFeel}), that choice is honored. Otherwise a modern default
	 * is used instead of the dated Metal LAF: FlatLaf (if its jar is on the
	 * classpath) for the most contemporary appearance, falling back to the
	 * built-in Nimbus LAF which is already far more modern than Metal.
	 * <p>
	 * This is best-effort: any failure to install a modern LAF simply leaves the
	 * previous (default) look &amp; feel in place, so the app always starts.
	 */
	private static void applyModernLookAndFeel() {
		...
		// overrides Aqua for every Swing widget in the app while the OS still
		// draws native window decorations and menus, which looks inconsistent and
		// loses the familiar Mac appearance users expect. Mac users who want the
		// bundled themes can still pick one from View|Theme.
		if (OSPRuntime.isMac()) {
			return;
		}
		// No explicit choice: prefer FlatLaf if it is available on the classpath.
		// FlatLightLaf comes first because Tracker's own panels and track colors are
		// designed for a light background.
		String[] flatLafClasses = { "com.formdev.flatlaf.FlatLightLaf",
				"com.formdev.flatlaf.FlatDarkLaf" };
		for (String cls : flatLafClasses) {
			try {
				Class.forName(cls);
				...
				// not present or failed - try next
			}
		}
		// Fall back to the built-in Nimbus LAF (modern, no extra dependency).
		try {
			UIManager.setLookAndFeel("javax.swing.plaf.nimbus.NimbusLookAndFeel");
		} catch (Throwable t) {
			// leave whatever default is installed
		}
	}

	/**
	 * Applies a few FlatLaf-specific tweaks for a cleaner, more modern look:
	 * rounded corners, roomier controls, unified tab and scrollbar styling and a
	 * softer accent colour. Every change is optional, so failures are ignored.
	 */
	private static void applyFlatLafTweaks() {
		try {
			javax.swing.UIDefaults d = UIManager.getDefaults();
			d.put("Component.arc", 8); //$NON-NLS-1$
			d.put("Button.arc", 10); //$NON-NLS-1$
			d.put("TextComponent.arc", 8); //$NON-NLS-1$
			d.put("ProgressBar.arc", 8); //$NON-NLS-1$
			d.put("ScrollBar.thumbArc", 999); //$NON-NLS-1$
			d.put("ScrollBar.thumbInsets", new java.awt.Insets(2, 2, 2, 2)); //$NON-NLS-1$
			d.put("ScrollBar.width", 12); //$NON-NLS-1$
			d.put("TabbedPane.showTabSeparators", true); //$NON-NLS-1$
			d.put("TabbedPane.tabHeight", 28); //$NON-NLS-1$
			d.put("TabbedPane.tabType", "underlined"); //$NON-NLS-1$ //$NON-NLS-2$
			d.put("TabbedPane.hasFullBorder", true); //$NON-NLS-1$
			d.put("ToolTip.background", new java.awt.Color(0x2b303b)); //$NON-NLS-1$
			d.put("ToolTip.foreground", java.awt.Color.WHITE); //$NON-NLS-1$
			d.put("Component.focusWidth", 1); //$NON-NLS-1$
			d.put("Component.innerFocusWidth", 1); //$NON-NLS-1$
			d.put("Table.showHorizontalLines", true); //$NON-NLS-1$
			d.put("Table.showVerticalLines", false); //$NON-NLS-1$
			d.put("Table.intercellSpacing", new java.awt.Dimension(0, 1)); //$NON-NLS-1$
			d.put("MenuItem.selectionType", "underline"); //$NON-NLS-1$ //$NON-NLS-2$
			d.put("SplitPane.dividerSize", 8); //$NON-NLS-1$
			d.put("SplitPane.centerOneTouchButtons", false); //$NON-NLS-1$
		} catch (Throwable t) {
			// purely cosmetic
		}
	}
...
//________________________________  static methods ____________________________
	...
	/**
	 * The look &amp; feel names offered in the View|Theme menu. The names are the
	 * LookAndFeel class names so the choice can be stored in the Tracker
	 * preferences and reapplied on the next start.
	 * <p>
	 * On macOS the native Aqua look &amp; feel is offered as well, otherwise a Mac
	 * user who switched to a bundled theme would have no way back to the native
	 * appearance. Two class names are tried because the Aqua implementation moved:
	 * {@code com.apple.laf.AquaLookAndFeel} on JDK 9+ and
	 * {@code javax.swing.plaf.mac.MacLookAndFeel} on JDK 8.
	 */
	static String[][] getThemes() {
		java.util.ArrayList<String[]> list = new java.util.ArrayList<>();
		if (OSPRuntime.isMac()) {
			String aqua = findAvailableLookAndFeel(
					"com.apple.laf.AquaLookAndFeel", //$NON-NLS-1$
					"javax.swing.plaf.mac.MacLookAndFeel"); //$NON-NLS-1$
			if (aqua != null) {
				list.add(new String[] { aqua, "Native (Aqua)" }); //$NON-NLS-1$
			}
		}
		list.add(new String[] { "com.formdev.flatlaf.FlatLightLaf", "Light" }); //$NON-NLS-1$ //$NON-NLS-2$
		list.add(new String[] { "com.formdev.flatlaf.FlatDarkLaf", "Dark" }); //$NON-NLS-1$ //$NON-NLS-2$
	...
	/**
	 * Returns the first of the given LookAndFeel class names that can actually be
	 * instantiated on this JVM, or null if none can.
	 *
	 * @param names candidate class names, most preferred first
	 * @return an available class name, or null
	 */
	private static String findAvailableLookAndFeel(String... names) {
		for (String name : names) {
			try {
				...
				// not available on this JVM - try the next
			}
		}
		return null;
	}
	...
	/**
	 * True when the given look &amp; feel is the platform's native one, which must
	 * not have the FlatLaf cosmetic overrides applied to it.
	 *
	 * @param className a LookAndFeel class name
	 * @return true for the macOS Aqua / Mac look &amp; feel
	 */
	private static boolean isNativeLookAndFeel(String className) {
		return className != null && (className.startsWith("com.apple.laf") //$NON-NLS-1$
				|| className.startsWith("javax.swing.plaf.mac")); //$NON-NLS-1$
	}

	/**
	 * Gets the class name of the look &amp; feel currently in effect.
	 *
	 * @return the look &amp; feel class name
	 */
	static String getCurrentTheme() {
		javax.swing.LookAndFeel lf = UIManager.getLookAndFeel();
		return lf == null ? "" : lf.getClass().getName(); //$NON-NLS-1$
	}

	/**
	 * Switches the application theme at runtime and remembers the choice.
	 * <p>
	 * Changing the look &amp; feel of a live Swing application is never perfect
	 * for components that cache their UI delegates, so after installing the new
	 * look &amp; feel every window is updated and revalidated. Failures are
	 * reported but never fatal.
	 *
	 * @param className the LookAndFeel class name
	 * @return true if the theme was changed
	 */
	static boolean setTheme(String className) {
		if (className == null || className.equals(getCurrentTheme())) {
			return false;
		}
		try {
			if (isNativeLookAndFeel(className)) {
				...
				if (className.startsWith("com.formdev.flatlaf")) { //$NON-NLS-1$
					applyFlatLafTweaks();
				}
			}
		} catch (Throwable t) {
			OSPLog.warning("unable to set look & feel " + className + ": " + t); //$NON-NLS-1$ //$NON-NLS-2$
			return false;
		}
...
//	    if (testOn) latestVersion = null; // for testing only
	...
	/**
	 * Registers the video engine (Xuggle) with the video I/O layer.
	 * <p>
	 * The Xuggle video type is registered by {@code XuggleVideo}'s static
	 * initializer, which only runs when that class is first loaded. Nothing used
	 * to load it early, so whether a video could be opened depended on incidental
	 * class-loading order: when the Xuggle jar was on the classpath the type was
	 * usually registered, and when only the classes bundled inside tracker.jar
	 * were available it never was, so every video was reported as an unsupported
	 * format. Loading it here makes video work regardless of how the jar was
	 * launched.
	 * <p>
	 * This is best-effort: if the engine is unavailable, Tracker still starts and
	 * simply cannot open videos.
	 */
	private static void registerVideoEngine() {
		if (OSPRuntime.isJS)
			return;
		try {
			Class.forName("org.opensourcephysics.media.xuggle.XuggleVideo"); //$NON-NLS-1$
		} catch (Throwable t) {
			OSPLog.fine("video engine unavailable: " + t); //$NON-NLS-1$
		}
	}
...
//  	}
	...
	private static void start(String[] args) {
		...
		// add Tracker's own fit functions (tanh terminal velocity, drag saturation)
		TrackerFits.register();
