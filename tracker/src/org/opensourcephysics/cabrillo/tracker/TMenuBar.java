...
//			viewMenu.removeAll();
	...
	/**
	 * Gets (creating if necessary) the View|Theme submenu that switches the
	 * application look &amp; feel.
	 *
	 * @return the theme submenu
	 */
	private JMenu getThemeMenu() {
		if (view_themeMenu == null) {
			view_themeMenu = new JMenu(TrackerRes.getString("TMenuBar.Menu.Theme")); //$NON-NLS-1$
			ButtonGroup group = new ButtonGroup();
			for (String[] theme : Tracker.getThemes()) {
				final String className = theme[0];
				JRadioButtonMenuItem item = new JRadioButtonMenuItem(theme[1]);
				// remember the class name on the item so the refresh below can
				// match by identity instead of by position (the available theme
				// list is platform-dependent, so positions can shift)
				item.setActionCommand(className);
				item.setSelected(className.equals(Tracker.getCurrentTheme()));
				item.addActionListener((e) -> Tracker.setTheme(className));
				group.add(item);
				view_themeMenu.add(item);
			}
		}
		return view_themeMenu;
	}

	/**
	 * Refreshes the theme menu so the currently installed look &amp; feel is
	 * checked.
	 */
	protected void refreshThemeMenu() {
		if (view_themeMenu == null)
			return;
		String current = Tracker.getCurrentTheme();
		for (int i = 0; i < view_themeMenu.getItemCount(); i++) {
			JMenuItem item = view_themeMenu.getItem(i);
			if (item == null)
				continue;
			String className = item.getActionCommand();
			item.setSelected(className != null && className.equals(current));
		}
	}
