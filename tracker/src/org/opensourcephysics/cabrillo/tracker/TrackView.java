...
public abstract class TrackView extends JScrollPane implements PropertyChangeListener {
  ...
  private static final String[] panelProps = {
	...
	public String getName() {
		// getName() may be called while this view is still being constructed, or
		// after its track has been disposed. Certain look & feels (Nimbus/Synth)
		// call Component.getName() while installing UI defaults, so this must
		// never throw.
		TTrack track = getTrack();
		return track == null ? super.getName() : track.getName();
	}
	...
	public Icon getIcon() {
		if (trackIcon == null) {
			...
			if (track == null)
				return null;
			trackIcon = track.getIcon(21, 16, "point"); //$NON-NLS-1$
		}
		return trackIcon;
	}
