...
public void back() {
   ...
   * <p>
   * The deferred {@code setFrameNumber} is deliberate (it avoids re-entrancy from
   * the slider), but the inherited {@code stepNumber} field must still be updated
   * <b>now</b>. {@link #step()} and {@link #back()} compute their target as
   * {@code stepNumber +/- 1}, and {@link #getStepNumber()} derives the step from
   * the video's frame number, which is only applied later. Leaving the field stale
   * meant that a second click before the deferred frame change had run computed
   * from the old step: stepping back landed on the step it had just come from, so
   * the frame appeared to move forward. Stepping back twice in quick succession
...
public void setStepNumber(int n) {
    ...
    // keep the inherited state consistent immediately, so a subsequent step or
    // back click starts from the step just requested rather than a stale one
    stepNumber = stepNum;
    videoFrameNumber = clip.stepToFrame(stepNum);
